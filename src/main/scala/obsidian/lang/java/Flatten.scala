...
object Flatten {
	...
	*/
	...
	enum FlatResult[+A] {
		case FlatError(msg:String) extends FlatResult[Nothing]
		case FlatOk[A](result:A) extends FlatResult[A]
	}

	import FlatResult.*

	given flatResultFunctor: Functor[FlatResult] = new Functor[FlatResult] {
		def map[A, B](fa: FlatResult[A])(f: A => B): FlatResult[B] = fa match {
			case FlatError(s) => FlatError(s)
			case FlatOk(a)    => FlatOk(f(a))
		}
	}

	given flatResultApplicative: ApplicativeError[FlatResult, String] = new ApplicativeError[FlatResult, String] {
	...
	given flatResultMonadError(using app:ApplicativeError[FlatResult, String]): MonadError[FlatResult, String] = new MonadError[FlatResult, String] {
	...
	def flatMethodDecl(a:MethodDecl)(using m:MonadError[FlatState, String]):FlatState[MethodDecl] = a match { 
	...
	def flatMethodBody(a:MethodBody)(using m:MonadError[FlatState, String]):FlatState[MethodBody] = a match { 
	...
	def tempVarDecls()(using m:MonadError[FlatState, String]):FlatState[List[BlockStmt]] = for {
		st <- get
		r  <- {
			val tyEnv = st.typeEnv
			def go(nn:(Name,Name)):FlatState[BlockStmt] = nn match {
	...
	def flatBlock(a:Block)(using m:Monad[FlatState]):FlatState[Block] = a match {
		case Block(blk_stmts) => for {
			f_blk_stmts <- blk_stmts.traverse(flatBlockStmt(_))
		} yield Block(f_blk_stmts.flatMap(x => x)) 
	}

	def flatStmt(a:Stmt)(using m:MonadError[FlatState, String]):FlatState[List[Stmt]] =  a match {
	...
	def flatSwitchBlock(switch_block:SwitchBlock)(using m:Monad[FlatState]):FlatState[SwitchBlock] = switch_block match {
		case SwitchBlock(label, blk_stmts) => for {
			f_blk_stmts <- blk_stmts.traverse(flatBlockStmt(_))
		} yield SwitchBlock(label, f_blk_stmts.flatMap(x => x))
	}

	def flatBlockStmt(blk_stmt:BlockStmt)(using m:Monad[FlatState]):FlatState[List[BlockStmt]] = blk_stmt match {
	...
	def flatCatch(c:Catch)(using m:Monad[FlatState]):FlatState[Catch] = c match {
	...
	trait LiftAll[A] {
		...
		*/
		def liftAll(a:A)(using m:MonadError[FlatState, String]):FlatState[(List[Stmt], A)]
	}

	object laOps {
		def liftAll[A](a:A)(using la:LiftAll[A], m:MonadError[FlatState, String]):FlatState[(List[Stmt], A)] = {
			la.liftAll(a)
		}
	}

	given ListLiftAllInstance[A](using la:LiftAll[A]):LiftAll[List[A]] = new LiftAll[List[A]] {
		override def liftAll(as: List[A])(using m: MonadError[FlatState,String]): FlatState[(List[Syntax.Stmt], List[A])] = {
	...
	given OptionLiftAllInstance[A](using la:LiftAll[A]):LiftAll[Option[A]] = new LiftAll[Option[A]] {
		override def liftAll(a: Option[A])(using m: MonadError[FlatState,String]): FlatState[(List[Syntax.Stmt], Option[A])] = a match {
	...
	given ExpLiftAllInstance:LiftAll[Exp] = new LiftAll[Exp] {
		override def liftAll(a: Syntax.Exp)(using m: MonadError[FlatState,String]): FlatState[(List[Syntax.Stmt], Syntax.Exp)] = {
	...
	given ForInitLiftAllInstance:LiftAll[ForInit] = new LiftAll[ForInit] {
		override def liftAll(a: Syntax.ForInit)(using m: MonadError[FlatState,String]): FlatState[(List[Syntax.Stmt], Syntax.ForInit)] = a match {
	...
	given VarDeclLiftAllInstance:LiftAll[VarDecl] = new LiftAll[VarDecl] {
		override def liftAll(a: Syntax.VarDecl)(using m: MonadError[FlatState,String]): FlatState[(List[Syntax.Stmt], Syntax.VarDecl)] = a match {
	...
	given VarInitLiftAllInstance:LiftAll[VarInit] = new LiftAll[VarInit] {
		override def liftAll(a: Syntax.VarInit)(using m: MonadError[FlatState,String]): FlatState[(List[Syntax.Stmt], Syntax.VarInit)] = a match {
	...
	given ArrayInitLiftAllInstance:LiftAll[ArrayInit] = new LiftAll[ArrayInit] {
		override def liftAll(a: Syntax.ArrayInit)(using m: MonadError[FlatState,String]): FlatState[(List[Syntax.Stmt], Syntax.ArrayInit)] = a match {
	...
	given LhsLiftAllInstance:LiftAll[Lhs] = new LiftAll[Lhs] {
		override def liftAll(a: Syntax.Lhs)(using m: MonadError[FlatState,String]): FlatState[(List[Syntax.Stmt], Syntax.Lhs)] = a match {
	...
	given ArrayIndexLiftAllInstance:LiftAll[ArrayIndex] = new LiftAll[ArrayIndex] {
		override def liftAll(a: Syntax.ArrayIndex)(using m: MonadError[FlatState,String]): FlatState[(List[Syntax.Stmt], Syntax.ArrayIndex)] = a match {
	...
	given MethodInvLiftAllInstance:LiftAll[MethodInv] = new LiftAll[MethodInv] {
		override def liftAll(a: Syntax.MethodInv)(using m: MonadError[FlatState,String]): FlatState[(List[Syntax.Stmt], Syntax.MethodInv)] = a match {
			case MethodInv(methodInv) => for {
				stmts_methodInv <- laOps.liftAll(methodInv) 
	...
	given MethodInvocationLiftAllInstance:LiftAll[MethodInvocation] = new LiftAll[MethodInvocation] {
		override def liftAll(a: Syntax.MethodInvocation)(using m: MonadError[FlatState,String]): FlatState[(List[Syntax.Stmt], Syntax.MethodInvocation)] = a match {
	...
	def addFormalParamToTypeEnv(formal_param:FormalParam)(using m: MonadError[FlatState,String]):FlatState[Unit] = formal_param match {
	...
	def addVarDeclsToTypeEnv(var_decls:List[VarDecl], mods:List[Modifier], ty:Type)(using m: MonadError[FlatState,String]):FlatState[Unit] = for {
		st <- get
		_  <- put(st.copy(
	...
	*/
	def mkAssignStmt(exp:Exp)(using m:MonadError[FlatState, String]):FlatState[(Name, Stmt)] = exp match {
	...
	def rename(n:Name)(using m:MonadError[FlatState,String]):FlatState[Name] = n match {
	...
	object naOps {
		def nestedAssignment[A](a:A)(using d:NestedAssignment[A]):Option[(Exp, Exp => A)] = {
			d.nestedAssignment(a)
		}
	}

	given ExpNestedAssignmentInstance:NestedAssignment[Exp] = new NestedAssignment[Exp] {
	...
	given LhsNestedAssignmentInstance:NestedAssignment[Lhs] = new NestedAssignment[Lhs] {
	...
	given FieldAccessAssignmentInstance:NestedAssignment[FieldAccess] = new NestedAssignment[FieldAccess] {
	...
	given ArrayInitNestedAssignmentInstance:NestedAssignment[ArrayInit] = new NestedAssignment[ArrayInit] {
	...
	given VarInitNestedAssignmentInstance:NestedAssignment[VarInit] = new NestedAssignment[VarInit] {
	...
	given MethodInvocationNestedAssignmentInstance:NestedAssignment[MethodInvocation] = new NestedAssignment[MethodInvocation] {
	...
	given ArrayIndexNestedAssignmentInstance:NestedAssignment[ArrayIndex] = new NestedAssignment[ArrayIndex] {
	...
	given ListNestedAssignmentInstance[A](using na:NestedAssignment[A]):NestedAssignment[List[A]] = new NestedAssignment[List[A]] {
